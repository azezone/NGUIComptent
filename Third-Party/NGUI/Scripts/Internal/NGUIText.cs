...
static public class NGUIText
{
	...
	static public int CalculateOffsetToFit_Oscar (string text)
	{
		if (string.IsNullOrEmpty(text) || rectWidth < 1) return 0;
		...
		UnityEngine.Debug.Log (currentCharacterIndex);
		while (currentCharacterIndex < mSizes.size && remainingWidth > 0) {
			UnityEngine.Debug.Log (currentCharacterIndex);
			remainingWidth -= mSizes [++currentCharacterIndex];
		}
